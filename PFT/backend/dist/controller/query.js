...
async function getBudgetDetails(req, res, next) {
    try {
        const { userId } = (0, verifyToken_1.default)(req);
        const month = new Date().getMonth() + 1;
        const response = await db_1.default.sequelize.query(`SELECT a.category, a.amount AS budget, COALESCE(SUM(b.amount), 0) AS spent, a.amount - COALESCE(SUM(b.amount), 0) AS balance, COUNT(b.id) AS expense_count
...
async function getYearlyDetails(req, res, next) {
    try {
        const { userId } = (0, verifyToken_1.default)(req);
        const month = new Date().getMonth() + 1;
        const response = await db_1.default.sequelize.query(`SELECT YEAR(a.createdAt) AS year, SUM(a.amount) AS total_income, SUM(b.amount) AS total_expense FROM Incomes a JOIN Expenses b ON YEAR(a.createdAt) = YEAR(b.createdAt) WHERE a.userId = :userId GROUP BY YEAR(a.createdAt);`, {
            replacements: { userId: userId },
            type: sequelize_1.QueryTypes.SELECT
        });
        if (!response) {
